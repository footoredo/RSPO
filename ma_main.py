...
def _run(args, logger):
    ...
    agents = [[] for _ in range(num_copies)]
    main_agent_conns = [[] for _ in range(num_copies)]
    envs = [[] for _ in range(num_copies)]
    main_env_conns = [[] for _ in range(num_copies)]
    ...
    for i, agent in enumerate(env.agents):
        ...
        obs_len = env.observation_spaces[agent].shape[0]
        size = item_size * (obs_len + 4) * num_envs
        obs_buffer_size += size
        agent_obs_pos.append((obs_pos, obs_pos + obs_len))
        obs_pos += obs_len
        agent_obs_shms.append(shared_memory.SharedMemory(create=True, size=item_size * obs_len * num_copies * batch_size))

    obs_shm = shared_memory.SharedMemory(create=True, size=obs_buffer_size * num_copies)
    act_shms = []
    ...
    for i_copy in range(num_copies):
        if num_copies > 1:
            save_dir = mkdir2(args.save_dir, run_name, "copy-{}".format(i_copy))
        else:
        ...
        # print(item_size)
        obs_indices = []
        copy_obs_buffer_start = obs_buffer_size
        ...
        for i, agent in enumerate(env.agents):
            conn1, conn2 = mp.Pipe()
            main_agent_conns[i_copy].append(conn1)
            ...
            ap = Agent(i, env.agents[i], thread_limit=thread_limit, logger=logger.info, args=args,
                       ...
                       obs_start=agent_obs_pos[i][0], obs_end=agent_obs_pos[i][1],
                       ...
                       backend=BACKEND,
                       agent_obs_shm=agent_obs_shms[i],
            ...
            # print("123123123", i)
            agents[i_copy].append(ap)
            ap.start()

        for i in range(num_envs):
            conn1, conn2 = mp.Pipe()
            main_env_conns[i_copy].append(conn1)
            ev = Environment(i, logger.info, args, env=_make_env(), agents=env.agents, main_conn=conn2,
                             ...
                             obs_buffer_start=copy_obs_buffer_start,
                             i_copy=i_copy,
                             num_copies=num_copies)
            envs[i_copy].append(ev)
            ev.start()

    for i_copy in range(num_copies):
        for conn in main_agent_conns[i_copy]:
            conn.send(None)

        for conn in main_env_conns[i_copy]:
            conn.send(None)
    ...
    for i_copy in range(num_copies):
        statistics = dict()
        for i, agent in enumerate(env.agents):
            statistics[agent] = main_agent_conns[i_copy][i].recv()
        all_statistics.append(statistics)

        import joblib
        joblib.dump(statistics, os.path.join(save_dirs[i_copy], "statistics.obj"))

    result["statistics"] = squeeze(all_statistics)
    ...
    for i_copy in range(num_copies):
        ...
        if args.play:
            ...
            for i in range(num_agents):
                main_agent_conns[i_copy][i].send(args.render)
            ...
            while True:
                ...
                # st = time.time()
                for i, agent in enumerate(env.agents):
                    main_agent_conns[i_copy][i].send((obs[agent], dones[agent]))
                    actions[agent], strats[agent] = main_agent_conns[i_copy][i].recv()
        ...
        for i, agent in enumerate(agents[i_copy]):
            main_agent_conns[i_copy][i].send(None)
            agent.join()
            # print(agent)
        ...
        if args.play:
            import joblib
            joblib.dump(play_statistics, os.path.join(save_dirs[i_copy], "play_statistics.obj"))
            show_play_statistics(args.env_name, play_statistics)
        all_play_statistics.append(play_statistics)

        act_shms[i_copy].close()
        act_shms[i_copy].unlink()
    ...
    result["play_statistics"] = squeeze(all_play_statistics)
    result["save_dir"] = squeeze(save_dirs)
