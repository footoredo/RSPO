...
def main():
    ...
    for agent in env.agents:
        ...
        forward_net = torch.nn.Sequential(torch.nn.Linear(num_rep + num_actions, num_hidden),
                                          torch.nn.ReLU(),
                                          torch.nn.Linear(num_hidden, num_rep))
        inverse_net = torch.nn.Sequential(torch.nn.Linear(num_rep * 2, num_hidden),
                                          torch.nn.ReLU(),
        ...
        state_rep_optimizer = torch.optim.RMSprop(state_rep_net.parameters(), lr=1e-4, weight_decay=0.1)
        ...
        for i in range(50000):
            ...
            loss = 1.0 * loss_forward + 1 * loss_inverse
            ...
            if i % 1000 == 0:
                print(i, loss_forward.item(), loss_inverse.item(), loss.item())
